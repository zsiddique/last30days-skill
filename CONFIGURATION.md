...
3. **Self-hosted SearXNG instance** - when `LAST30DAYS_SEARXNG_URL` is set and no paid key is configured, the engine queries your SearXNG instance's JSON API directly. SearXNG aggregates results across real search engines (including keyed ones like Brave when the instance is configured with them), so this tier preempts both crawl4ai and the keyless DuckDuckGo floor below it while still yielding to a configured paid key. Force it explicitly with `--web-backend=searxng`. **Behavior change:** `LAST30DAYS_SEARXNG_URL` used to be consulted only as a last-resort fallback inside the keyless floor when DuckDuckGo returned nothing; it is now a first-class backend that runs instead of crawl4ai/DuckDuckGo whenever it's set. **Precondition:** the instance must have `json` enabled in `search.formats` (in `settings.yml`); stock SearXNG ships with only `[html]` and will return HTTP 403 for JSON requests until you add it.
4. **Keyless engine floor** - zero-key web search (DuckDuckGo, then Startpage) and zero-key page fetch (Jina Reader). Runs only when the agent session has **no** host web search, no paid key, **and** no `LAST30DAYS_SEARXNG_URL` configured, so headless/cron and hosts without a search tool still get general-web coverage. Force it explicitly with `--web-backend=keyless`; when forced this way, DuckDuckGo/Startpage still fall back to a configured SearXNG instance internally as a last resort before returning nothing.
...
| `LAST30DAYS_SEARXNG_URL=<base-url>` | Optional. Promotes a self-hosted SearXNG instance to a first-class web-search backend (tier 3 above); requires `json` in the instance's `search.formats`. |
